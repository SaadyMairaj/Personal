...
class OneArm(Frame):
    ...
    def SaveToDatabaseContinued(self, entry, dlg):
        ...
        # task = (str(SaveAs), )
        sql = "SELECT OneArmId FROM OneArm Where Name = ?;"
        ...
        if ResultSet:
            ...
            for data in range(len(ResultSet)):
                sql = "DELETE FROM OneArmValue Where OneArmIdFK = ?;"
                cursor.execute(sql, ResultSet[data])

                sql = "DELETE FROM OneArm WHERE OneArmId = ?;"
                cursor.execute(sql, ResultSet[data])
    ...
    def WriteToExcelFile(self, window):
        ...
        try:
            ...
            worksheet.write_string(3, 0, Heading[0], WrapText)
            worksheet.write_string(3, 1, Heading[1], WrapText)
            worksheet.write_string(3, 2, Heading[2], WrapText)
            worksheet.write_string(3, 3, Heading[3], WrapText)
            ...
            worksheet.write(cell, '=SUM(D7:D50)', BorderFormat)
